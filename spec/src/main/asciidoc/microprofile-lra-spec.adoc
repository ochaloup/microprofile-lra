...
* *Fail*: the client has canceled the LRA and activity has completed
...
1.  link:#java-annotations-for-lras[Java Annotations for LRAs] for controlling the lifecycle and membership of LRAs
2.  link:#lra-client-api[Client API] for use with containers that do not use Java annotations
3.  link:#java-based-lra-participant-registration-api[Java based LRA
participant registration API] to support services that do not use JAX-RS
...
link:../../../../api/src/main/java/org/eclipse/microprofile/lra/annotation/[API package].
...
The lifecycle of an LRA is controlled via the
link:../../../../api/src/main/java/org/eclipse/microprofile/lra/annotation/LRA.java[LRA annotation].
...
via link:#http-lra-header[request and response headers], for example
...
link:../../../../api/src/main/java/org/eclipse/microprofile/lra/client/IllegalLRAStateException.java[IllegalLRAStateException]
or InvalidStateException). Otherwise the
...
link:../../../../api/src/main/java/org/eclipse/microprofile/lra/client/LRAClient.java[LRA client API]
can be instantiated directly or injected if the client is using CDI:
...
compensation activities the developer can register participants directly using an instance of
`link:../../../../api/src/main/java/org/eclipse/microprofile/lra/participant/LRAManagement.java[LRAManagement]`.
This interface requires that a participant be a serializable java class conforming to the interface:
...
To support the crash recovery requirements of this specification the application SHOULD provide
a class that is capable of recreating a participant from its serialized form by registering an
instance of LRAParticipantDeserializer:
...
The registration mechanism uses an instance of LRAManagement:
